...
def ppo_train_epoch(epoch, loader, iters, old_actor_model, ref_model, actor_scheduler, critic_scheduler, reward_model, reward_tokenizer, start_step=0, wandb=None):
    ...
    for step, batch in enumerate(loader, start=start_step + 1):
        ...
        with torch.no_grad():           # 在 eval 状态下，actor生成answer
        ...
        responses_text = [tokenizer.decode(gen_out[i, prompt_length:], skip_special_tokens=True) for i in range(len(prompts))]      # 删除了前置的prompt
        ...
        advantages = rewards - values.detach()  # [B]       reward - critic_last_value
        ...
        resp_mask = torch.arange(seq_len, device=gen_out.device).unsqueeze(0) >= prompt_length - 1      # 输出示例：如果prompt_length=5，seq_len=10，则掩码为[False, False, False, False, True, True, True, True, True, True]
        final_mask = resp_mask & (~labels.eq(tokenizer.pad_token_id))  # [B, P+R-1]                     # 最后真实需要计算mask的部分，一个是prompt屏蔽，一个是结束后的pad屏蔽
        ...
        ratio = torch.exp(actor_logp - old_logp)  # [B]             # 重要性采样r_theta，pai_theta / pai_old_theta
        surr1 = ratio * advantages  # [B]                           # L_clip第一个公式，r_theta * Advantage
        surr2 = torch.clamp(ratio, 1.0 - args.clip_epsilon, 1.0 + args.clip_epsilon) * advantages  # [B]        # L_clip第二个公式，裁剪
        policy_loss = -torch.min(surr1, surr2).mean()  # scalar     # 计算L_clip结果
...
if __name__ == "__main__":
    ...
    parser.add_argument("--save_dir", type=str, default="/home/boran.lbr/gitspace/github/minimind/out", help="模型保存目录")
    ...
    parser.add_argument("--data_path", type=str, default="/home/boran.lbr/gitspace/github/minimind/dataset/minimind_dataset/rlaif-mini.jsonl", help="RLAIF数据路径")
    ...
    parser.add_argument("--reward_model_path", type=str, default="/home/boran.lbr/LLM_model/internlm2-1_8b-reward", help="Reward模型路径")
    ...
    base_weight = 'none'
    actor_model = MiniMindForCausalLM.from_pretrained(
        "E:/code/minimind_old/save/hf/SFT_v0", 
        local_files_only=True, trust_remote_code=True).to(args.device)
    tokenizer = AutoTokenizer.from_pretrained("E:\\code\\minimind_old\\model", local_files_only=True, trust_remote_code=True)

    # Old Actor模型
    old_actor_model = MiniMindForCausalLM.from_pretrained(
        "E:/code/minimind_old/save/hf/SFT_v0", 
        local_files_only=True, trust_remote_code=True).to(args.device)
    old_actor_model = old_actor_model.eval().requires_grad_(False)

    # Reference模型
    ref_model = MiniMindForCausalLM.from_pretrained(
        "E:/code/minimind_old/save/hf/SFT_v0", 
        local_files_only=True, trust_remote_code=True).to(args.device)
    ...
    ckp = 'E:/code/minimind_old/save/full_sft_data512_10ep_len512_768.pth'
